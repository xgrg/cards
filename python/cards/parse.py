from cards import preamble as pr
...
def clean_json(j):
    ...
    for k, v in j.items():
        ...
        if 'Conditions' in v.keys():
            sc['conditions'] = table.parse_dict(v['Conditions'])
        if 'Action' in v.keys():
        ...
        if 'Choices' in v.keys():
            ...
            for k1, v1 in v['Choices'].items():
                choice_parts = pr.split_choice(v1)
                choice_dict = table.parse_dict(choice_parts[0])
                if len(choice_parts) > 1:
                    choice_dict.append(('@if', table.parse_dict(choice_parts[1])))
...
def markdown_to_json(md_fp, rebuild_md=False):
    ...
    md_wo_breaklines = pr.remove_minuslines(md)
    # jsvn.beautify_md(md_fp)
    preamble, body = pr.get_preamble(md_wo_breaklines)
